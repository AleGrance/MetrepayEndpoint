module.exports = (app) => {
  const Pagos_links_metrepay = app.db.models.Pagos_links_metrepay;
  const apiToken = "35dd7b33-3c2a-48a4-827f-042e57c9c3b8";

  app
    .route("/api/pagos_links_metrepay")
    .get((req, res) => {
    ...
    .post((req, res) => {
      ...
      if (req.headers.apitoken === apiToken) {
        Pagos_links_metrepay.create(req.body.data)
  ...
  app
    .route("/api/pagos_links_metrepay/:customIdentifier")
    .get((req, res) => {
      if (!req.headers.apitoken) {
        return res.status(403).send({
          error: "Forbidden",
    ...
    .put((req, res) => {
      ...
      if (req.headers.apitoken === apiToken) {
        Pagos_links_metrepay.update(req.body, {
          where: req.params,
        })
